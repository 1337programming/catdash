...
import {Serializer} from 'platform-tools/rx-extensions/src/Serializers/Serializer';
...
import {FilesList} from 'platform-tools/rx-extensions/src/interfaces';
    ...
    Serializer.parseData(request.method, url, request.data, request.files, (data) => {
      ...
      xhr.onreadystatechange = () => {
        ...
        if ((xhr.status < 200 || xhr.status > 300) && xhr.status !== 304) {
          let error = new Error('Error loading resource ' + request.url + ' - server replied: '
