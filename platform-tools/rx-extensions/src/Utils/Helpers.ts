...
export class Helpers {
  ...
  public static addEventListener(el: any, event: string, listener: (e: Event, target: HTMLElement) => void,
                                 checkFn?: (target: HTMLElement) => boolean): EventListener {
    let fn = (e: Event) => {
      if (!checkFn) {
  ...
  public static toASCII(str: string): string {
    ...
    for (let toLetter in diacritics) {
      if (diacritics.hasOwnProperty(toLetter)) {
        for (let i: number = 0; i < diacritics[toLetter].length; i++) {
